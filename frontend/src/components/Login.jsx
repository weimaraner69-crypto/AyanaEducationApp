...
const Login = ({ profiles, onLogin, onParentLogin }) => (
  <div className="min-h-screen bg-[#F8FAFC] px-4 py-8 sm:p-6 flex flex-col items-center justify-center text-center">
    <div className="w-full max-w-sm sm:max-w-md space-y-8 sm:space-y-12">
      ...
      <div className="grid grid-cols-1 gap-4 text-slate-900">
        {profiles.map(p => (
          <button
            key={p.id}
            onClick={() => onLogin(p)}
            className="group p-5 sm:p-6 bg-white border-2 border-transparent hover:border-indigo-500 rounded-[2.5rem] shadow-sm hover:shadow-xl transition-all flex items-center gap-4 sm:gap-6 text-left"
          >
            <div className={`p-4 sm:p-5 bg-${p.color}-50 text-${p.color}-600 rounded-3xl group-hover:bg-${p.color}-500 group-hover:text-white transition-all`}>
              <p.icon size={32} />
            </div>
            <div className="flex-1">
              <h3 className="text-2xl font-black">{p.name}</h3>
              <div className="flex items-center gap-2 mt-1">
                <span className="text-sm font-black text-slate-500 bg-slate-100 px-2 py-0.5 rounded-md">{p.age}歳</span>
                <span className={`text-sm font-black ${p.stage === 'es' ? 'text-amber-600' : 'text-indigo-600'} uppercase tracking-widest`}>{p.gradeLabel}</span>
              </div>
            </div>
