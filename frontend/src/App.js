...
import { 
  Camera, BookOpen, CheckCircle2, AlertCircle, ArrowRight, RotateCcw, Home, 
  BrainCircuit, FileText, GraduationCap, Lightbulb, Search, Info, ChevronRight, 
  X, Upload, History, LayoutGrid, ShieldCheck, Zap, Quote, Sparkles, FlaskConical, 
  Globe, Languages, Library, Database, CheckCircle, Archive, UserCircle, Settings, 
  Lock, Unlock, Share2, UserPlus, Cloud, TrendingUp, HelpCircle, ClipboardList, 
  BookMarked, RefreshCw, LogOut, Pencil, Calculator, Palette, HeartPulse, Hammer, Sun,
  User, Key, Globe2, Loader2, ExternalLink, Fingerprint, ShieldAlert, Scissors, Link2
} from 'lucide-react';
...
const App = () => {
  ...
  const calculateInfo = (birthDateStr) => {
    ...
    return { age, gradeLabel, stage };
  };

  const profiles = useMemo(() => {
    const sonInfo = calculateInfo('2014-04-09');
    const daughterInfo = calculateInfo('2012-05-29');
    return [
      { id: 'son_masamune', name: '政宗', stage: sonInfo.stage, color: 'amber', icon: Pencil, age: sonInfo.age, gradeLabel: sonInfo.gradeLabel },
      { id: 'daughter_ayana', name: '文菜', stage: daughterInfo.stage, color: 'indigo', icon: BookOpen, age: daughterInfo.age, gradeLabel: daughterInfo.gradeLabel }
    ];
  }, [TODAY]);
      ...
      <div className="min-h-screen bg-[#F8FAFC] p-6 flex flex-col items-center text-slate-900">
        ...
        <div className="w-full max-w-md space-y-6 text-slate-800">
          <div className="bg-white p-10 rounded-[2.5rem] shadow-sm border-b-8 border-slate-100 min-h-[220px] flex items-center relative overflow-hidden text-center transition-all">
             <div className="absolute top- left-0 w-2 h-full bg-indigo-500/10"></div>
